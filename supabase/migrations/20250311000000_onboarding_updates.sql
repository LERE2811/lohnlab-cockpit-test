-- Migration for onboarding updates
-- This migration adds new tables and columns to support the updated onboarding flow
...
-- Create a table for onboarding progress
...
-- Update the ansprechpartner table to add new fields
ALTER TABLE "public"."ansprechpartner" 
ADD COLUMN IF NOT EXISTS "category" "text",
ADD COLUMN IF NOT EXISTS "has_cockpit_access" boolean DEFAULT false,
ADD COLUMN IF NOT EXISTS "company_name" "text",
ADD COLUMN IF NOT EXISTS "phone" "text";

-- Create a function to extract onboarding data
...
BEGIN
    IF NEW.onboarding_completed = true AND OLD.onboarding_completed = false THEN
        -- Extract data from onboarding_progress
...
-- Create a trigger to call the function when onboarding is completed
CREATE TRIGGER "trigger_onboarding_complete"
    AFTER UPDATE OF "onboarding_completed" ON "public"."subsidiaries"
    FOR EACH ROW
...
CREATE POLICY "Allow all operations for authenticated users" ON "public"."billing_info"
    USING (true)
    WITH CHECK (true);

-- Create policies for onboarding_progress
CREATE POLICY "Allow all operations for authenticated users" ON "public"."onboarding_progress"
    USING (true)
    WITH CHECK (true);
